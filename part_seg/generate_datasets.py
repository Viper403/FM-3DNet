...
def pairing(start_index, end_index,partition):
    data,_,_ = load_data_partseg(partition)
...
def data_preprocess(partition):
    ...
    if partition=='trainval':
        # ximin
        start_index_list = [0, 2048, 4096, 6144, 8192,9840,11888]
        end_index_list = [2048, 4096, 6144, 8192, 9840,11888,14007]
        for h5_index in range(len(start_index_list)):
            point_clouds_list, transformed_point_clouds_list, translation_list, rotation_list = pairing(start_index_list[h5_index], end_index_list[h5_index],partition)
            # save H5files
            saveH5(point_clouds_list, transformed_point_clouds_list, translation_list, rotation_list, "trainDataPart_{}.h5".format(str(h5_index)))
    else:
        # ximin
        #point_clouds_list, transformed_point_clouds_list, translation_list, rotation_list = pairing(0,5)
        point_clouds_list, transformed_point_clouds_list, translation_list, rotation_list = pairing(0,2874,partition)
        saveH5(point_clouds_list, transformed_point_clouds_list, translation_list, rotation_list,"testDataPart_0.h5")
if __name__ == '__main__':
    #data_preprocess('trainval')
    #data_preprocess('test')
    #readH5()
    h5_name = "./trainDataPart_0.h5"
    f = h5py.File(h5_name)
    pcs = f.get("point_clouds")
